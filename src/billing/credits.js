...
export const CREDIT_PACKS = [
  { id: "starter", label: "Starter Pack", priceUsd: 5.00,  credits: 5_000,  highlight: false },
  { id: "value",   label: "Value Pack",   priceUsd: 10.00, credits: 10_000, highlight: true  },
  { id: "pro",     label: "Pro Pack",     priceUsd: 15.00, credits: 15_000, highlight: false },
];
...
export async function fetchCreditBalance() {
  ...
  if (!jwt) return null;
  try {
    const res = await fetch("/api/balance", { headers: { Authorization: `Bearer ${jwt}` } });
    if (res.status === 401) { clearStoredJWT(); return null; }
...
export async function verifyStripeSession(sessionId) {
  ...
  // Webhook may not have fired yet — poll for up to 12s
  if (initialCredits === 0) {
    for (let i = 0; i < 6; i++) {
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 8, ...style }}>
      ...
      {onBuyClick && (
        <button onClick={onBuyClick} style={{
          background:   low ? "linear-gradient(135deg, #991b1b, #ef4444)" : "linear-gradient(135deg, #7c3aed, #a78bfa)",
          border: "none", color: "white", borderRadius: 8, padding: "7px 16px",
          fontSize: 13, fontWeight: 600, cursor: "pointer",
          boxShadow: "0 2px 8px rgba(124,58,237,0.3)", display: "flex", alignItems: "center", gap: 5,
        }}>
...
export function CreditPackModal({ onClose }) {
  ...
  const buy = async (packId) => {
    setPurchasing(packId); setError(null);
    try {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.65)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999 }}>
      <div style={{ background: "rgba(15,23,42,0.98)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 14, padding: "28px 24px", minWidth: 340, maxWidth: 420 }}>
        <div style={{ fontSize: 15, fontWeight: 700, color: "#e2e8f0", marginBottom: 6 }}>Buy AiRA Credits</div>
        <div style={{ fontSize: 12, color: "#64748b", marginBottom: 20, lineHeight: 1.5 }}>
          ...
          {!BILLING_ENABLED && <span style={{ color: "#f59e0b", display: "block", marginTop: 6 }}>Demo mode — no real charge.</span>}
        </div>
        {error && <div style={{ color: "#f87171", fontSize: 12, marginBottom: 12 }}>{error}</div>}
        {done ? (
          <div style={{ background: "rgba(134,239,172,0.08)", border: "1px solid rgba(134,239,172,0.2)", borderRadius: 8, padding: "12px 14px", color: "#86efac", fontSize: 13 }}>
            ✓ {done.creditsAdded.toLocaleString()} credits added — new balance: {done.newBalance.toLocaleString()}
          </div>
        ) : CREDIT_PACKS.map(pack => (
          <div key={pack.id} style={{ border: `1px solid ${pack.highlight ? "rgba(124,58,237,0.4)" : "rgba(255,255,255,0.08)"}`, background: pack.highlight ? "rgba(124,58,237,0.06)" : "transparent", borderRadius: 8, padding: "14px 16px", marginBottom: 10, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
              <div style={{ fontSize: 13, fontWeight: 600, color: "#e2e8f0" }}>
                {pack.label}{pack.highlight && <span style={{ marginLeft: 8, fontSize: 10, color: "#a78bfa", fontWeight: 700 }}>BEST VALUE</span>}
              </div>
              <div style={{ fontSize: 11, color: "#64748b", marginTop: 2 }}>{pack.credits.toLocaleString()} credits</div>
            </div>
            <button onClick={() => buy(pack.id)} disabled={!!purchasing} style={{ background: purchasing === pack.id ? "rgba(255,255,255,0.05)" : "linear-gradient(135deg,#7c3aed,#a78bfa)", border: "none", color: purchasing === pack.id ? "#475569" : "white", borderRadius: 6, padding: "6px 16px", fontSize: 13, fontWeight: 700, cursor: purchasing ? "wait" : "pointer", minWidth: 72 }}>
              {purchasing === pack.id ? "…" : `$${pack.priceUsd.toFixed(2)}`}
            </button>
          </div>
        ))}
        <button onClick={onClose} style={{ background: "transparent", border: "1px solid rgba(255,255,255,0.1)", color: "#64748b", borderRadius: 6, padding: "5px 14px", fontSize: 11, cursor: "pointer", marginTop: 12, width: "100%" }}>
          {done ? "Close" : "Cancel"}
        </button>
      </div>
    </div>
