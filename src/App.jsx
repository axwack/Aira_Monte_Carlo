...
/**
 * Is the feedback email transport configured?
 *
 * All three REACT_APP_EMAILJS_* values are inlined by CRA at BUILD time, not read
 * at runtime — so they must exist wherever the bundle is BUILT: a local
 * `.env.local` for `npm start`, and the Cloudflare Pages build environment for
 * the deployed site. `.env*` is gitignored, so a fresh clone or a second machine
 * has none of them, and the feedback button previously failed there with
 * "The public key is required" plus an alert telling the USER to open a console.
 *
 * See .env.example for the variable names and where each value comes from.
 */
...
export default function AiRAForecaster() {
  ...
  useEffect(() => {
    // Guarded: init(undefined) throws "The public key is required" the moment the
    // three REACT_APP_EMAILJS_* vars are absent — which is the DEFAULT state on a
    // fresh clone, because `.env*` is gitignored and therefore does not travel
    // between machines. See EMAILJS_CONFIGURED below and .env.example.
    if (EMAILJS_CONFIGURED) emailjs.init(process.env.REACT_APP_EMAILJS_USER_ID);
  }, []);
  ...
  const handleSendFeedback = async () => {
    ...
    // Fail honestly and EARLY when the transport isn't configured. Previously
    // this called emailjs.send(undefined, undefined, …), which rejected, and the
    // catch below told the user to "check the console for details" — asking a
    // retiree to open devtools for our missing build variable, and losing their
    // written feedback in the process.
    if (!EMAILJS_CONFIGURED) {
      const missing = [
        !process.env.REACT_APP_EMAILJS_USER_ID && "REACT_APP_EMAILJS_USER_ID",
        !process.env.REACT_APP_EMAILJS_SERVICE_ID && "REACT_APP_EMAILJS_SERVICE_ID",
        !process.env.REACT_APP_EMAILJS_TEMPLATE_ID && "REACT_APP_EMAILJS_TEMPLATE_ID",
      ].filter(Boolean);
      console.error("[feedback] EmailJS not configured. Missing build vars:", missing.join(", "),
        "— set them in .env.local for local dev and in the Cloudflare Pages build environment for production, then rebuild. See .env.example.");
      // alert() to match this dialog's existing success/error pattern. The text
      // fields are deliberately NOT cleared here (only the success path clears
      // them), so the user's message survives and can be copied.
      alert(
        "Feedback email isn't set up on this build, so this can't be sent from here.\n\n" +
        "Your message is still in the box — please copy it and reach me through one of the links on the About page. Sorry for the detour."
      );
      return;
    }
    ...
    try {
      ...
      // Never ask an end user to open devtools. Keep their text so a retry or a
      // copy-paste elsewhere is still possible.
      alert(
        "Sorry — your feedback couldn't be sent just now (" + (error?.text || error?.message || "network or service error") + ").\n\n" +
        "Your message is still in the box. Please try again, or reach me through the About page links."
      );
    }
  };
