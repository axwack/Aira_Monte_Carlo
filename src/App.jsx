...
export const BUILD_TAG = "[main] v1.2.33 — Instrument /api/verify-session so the credits incident can be diagnosed instead of guessed at. 3 of the last 4 Stripe checkout nonces were never consumed: customers paid, the webhook credited D1 correctly, and their browser walked away with no JWT — so they saw a zero balance and could not spend anything. Every exit from verify-session now logs one JSON line tagged [verify-session] with a distinct reason code (missing_session_id, missing_nonce, no_db_binding, nonce_not_consumed, stripe_lookup_failed, payment_not_paid, no_stripe_customer) plus the request origin and elapsed ms. The nonce_not_consumed branch does one extra read to say WHICH of nonce_not_found / nonce_already_consumed / nonce_expired actually happened — three very different bugs that one shared message was hiding, and the reason the customer-facing 'link has expired' copy was misleading. The success path logs too, because otherwise you cannot tell no failures from no traffic, which is what let this sit unnoticed. The nonce and the minted JWT are deliberately never logged (live credentials); the session id is, because it is already in the customer’s own URL and is what support needs to identify a purchase. Prior v1.2.32: typeable sidebar values + credits panel no longer shows 0 when there is simply no session.";
export const BUILD_TIME = "2026-07-27T20:00:00Z";
