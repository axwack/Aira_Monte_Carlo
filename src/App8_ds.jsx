...
/* ════ TAX BRACKETS & HELPERS (moved to global) ════ */
...
// NEW: Shared tax calculator – replaces taxDragRate everywhere
...
function portReturn(age, rand, eqWeightPre = 91, eqWeightPost = 70) {
  const eqW = age < 62 ? eqWeightPre / 100 : eqWeightPost / 100;
  return (
...
// taxDragRate kept for backward compatibility but not used in new code
...
/* ════ MONTE CARLO ENGINE — with real tax, GK on pre‑tax spending ════ */
function runMC(p, endAge, N = 3000, seed = 42, useGK = true) {
  ...
  for (let i = 0; i < N; i++) {
    ...
    for (let y = 0; y < retYrs; y++) {
      ...
      const yr = baseYear + (age - p.currentAge);
      const r = portReturn(
        age,
        rand,
        p.preRetireEq || 91,
      ...
      const draw = need + taxResult.totalTax + hShock;
...
// runStress — same tax and GK logic as runMC
...
/* ════ ROTH CONVERSION EXPLORER — v6 Boldin-style, with pre‑tax withdrawal added ════ */
const RMD_DIV = {
  73: 30.4,
  74: 29.5,
  75: 28.5,
...
function buildRothExplorer(params = {}) {
  ...
  function runScenario(doConvert) {
    ...
    for (let age = retireAge; age <= 90; age++) {
      ...
      // Pre‑tax portion of portfolio withdrawal is taxable
      const preTaxPortion = Math.round(portDraw * 0.6);
      ...
      const incBC = baseInc + preTaxPortion + rmd;
      ...
      pT = Math.max(0, pT - rmd - conv - preTaxPortion) * (1 + gr);
...
/* ════ COMPONENTS (only IncomeMap changed, others unchanged) ════ */
...
// IncomeMap – now also displays stacked taxes (federal + state + IRMAA)
function IncomeMap({ p, inf }) {
  ...
  const incomeData = useMemo(() => {
    ...
    return Array.from({ length: yrs }, (_, i) => {
      ...
      const portDraw = Math.max(0, Math.round(sp) - ss - ab);
      // Calculate taxes using calcYearTax
      const tax = calcYearTax(
        age,
        yr,
        portDraw,
        ss,
      ...
      return {
        age,
        "Portfolio Draw": portDraw,
        "Social Security": ss,
        "Airbnb Net": ab,
        "Federal Tax": tax.fedTax,
        "State Tax": tax.stateTax,
        IRMAA: tax.irmaa,
      };
    });
  }, [p, inf]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      {/* NEW: Tax stacked bar chart */}
      <div className="chart-card">
        <div className="ct">
          Projected taxes · Federal + State + IRMAA (real dollars)
        </div>
        <ResponsiveContainer width="100%" height={220}>
          ...
          >
            ...
            <Legend
              wrapperStyle={{ fontSize: 10, color: "#64748b", paddingTop: 6 }}
            />
            <Bar dataKey="Federal Tax" stackId="tax" fill="#f87171" />
            <Bar dataKey="State Tax" stackId="tax" fill="#fb923c" />
            <Bar
              dataKey="IRMAA"
              stackId="tax"
              fill="#a78bfa"
        ...
        <div className="flag-i" style={{ marginTop: 8, fontSize: 10 }}>
          ℹ️ Taxes are calculated using current brackets, standard deduction,
          85% SS inclusion, and IRMAA surcharges.
          {p.twoHousehold
            ? " Two‑household (FL/Thailand) → 0% state tax."
            : " NJ state tax applied."}
        </div>
      </div>
    </div>
  );
...
// RothLadder component unchanged except it uses the updated buildRothExplorer
...
// ========== MECO-STYLE MONTE CARLO TAB (unchanged) ==========
...
export default function AiRAForecaster() {
  ...
  const [activeTab, setTab] = useState("monte");
  const [running, setRunning] = useState(false);
        ...
        <div className="hdr">
          <div>
            ...
            <div style={{ fontSize: 10, color: "#334155" }}>
              v5.9 · All bugs fixed · GK Guardrails · 80% Airbnb reliability ·
